...
export async function fetchCommentsPages(postId:number): Promise<number | null> {
  const supabase = await createClient();
...
export async function fetchPostById(postId: number, currentPage: number): Promise<PostDetail | null> {
  try {
    const supabase = await createClient();
