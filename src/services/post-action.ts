      ...
      const uploadResults = await Promise.allSettled(
        images.map(async (image) => {
          ...
          const { error: uploadError } = await supabase.storage
            .from('post-images')
            .upload(`images/${fileName}`, image, { cacheControl: '3600', upsert: false });
          ...
          await supabase.from('images').insert({ post_id: postId, image_url: publicUrlData.publicUrl });
        })
      );
    ...
    if (imagesToDelete.length > 0) {
      const deleteResults = await Promise.allSettled(
        imagesToDelete.map(async (url) => {
          const path = new URL(url).pathname.replace('/storage/v1/object/public/post-images/', '');
          const { error: deleteError } = await supabase.storage
            .from('post-images')
            .remove([path]);
  
          if (deleteError) {
            console.error(`스토리지에서 이미지 삭제 실패: ${path}`, deleteError.message);
  ...
  try {
    ...
    if (images && images.length > 0) {
      const imagePaths = images.map((img) =>
        new URL(img.image_url).pathname.replace('/storage/v1/object/public/post-images/', '')
      );
